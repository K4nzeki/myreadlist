...
CREATE POLICY "Users manage their own completion log"
ON public.completion_log FOR ALL TO authenticated
USING (auth.uid() = user_id)
WITH CHECK (auth.uid() = user_id);

CREATE INDEX completion_log_user_month_idx ON public.completion_log (user_id, month);

CREATE TRIGGER completion_log_set_updated_at
BEFORE UPDATE ON public.completion_log
