-- Replace the single "own rows only" policy with separate ones per action
DROP POLICY IF EXISTS "Users manage their own entries" ON public.entries;

-- Anyone signed in can VIEW any entries
CREATE POLICY "Entries are viewable by everyone"
  ON public.entries FOR SELECT
  TO authenticated
  USING (true);

-- Only the owner can add, edit, or delete their own entries
...
-- Let everyone see usernames too, so the user list has something to show
